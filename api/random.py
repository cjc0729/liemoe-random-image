...
class handler(BaseHTTPRequestHandler):
    def request_handler(self, code, content_type, remain_num, opt):
    ...
    def do_GET(self):
        ...
        if withSpeedLimit:
            if limit(userip):
                ...
                self.request_handler(429, 'text/html;charset=utf-8', None, opt)
                return
        ...
        if only_check:
            ...
            self.request_handler(200, 'text/html;charset=utf-8', (maxNum - ip_block[0]), opt)
            return
        ...
        if setSort == "random":
            ...
            self.request_handler(404, 'text/html;charset=utf-8', None, opt)
            return
        ...
        if setNum == 1 and not setType == "json":
            ...
            if ip_block[0] > maxNum:
                ...
                self.request_handler(403, 'text/html;charset=utf-8', (maxNum - ip_block[0]), opt)
                return
            opt = read_data(setSort)
        elif setNum > 100 or setNum < 1:
            opt = report_error("请求失败", "400 Bad Request", "我无法理解您的请求。", "发生什么了？", "您提交的num参数不合法")
            self.request_handler(400, 'text/html;charset=utf-8', None, opt)
            return
        else:
            ...
            setType = "json"
        if setType == "text":
            self.request_handler(200, 'text/plain;charset=utf-8', (maxNum - ip_block[0]), opt)
        elif setType == "json":
            self.request_handler(200, 'application/json', (maxNum - ip_block[0]), opt)
        else:
